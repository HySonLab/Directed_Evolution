...
def get_slopes(n, mode="standard_alibi", verbose=False):
    ...
    Code has been adapted from the official ALiBi codebase at:
    ...
    if math.log2(n).is_integer():
        ...
        result = get_slopes_power_of_2(closest_power_of_2) + get_slopes(
...
class SpatialDepthWiseConvolution(nn.Module):
    ...
    def __init__(self, head_dim: int, kernel_size: int = 3):
        ...
        self.conv = nn.Conv1d(in_channels=head_dim,
...
class TranceptionBlockAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if self.attention_mode == "tranception":
            assert self.num_heads % 4 == 0, "Invalid number of heads. Tranception requires " \
                                            "the number of heads to be a multiple of 4."
    ...
    def prune_heads(self, heads):
        ...
        heads, index = find_pruneable_heads_and_indices(heads, self.num_heads,
                                                        self.head_dim, self.pruned_heads)
    ...
    def __init__(self, config, SDWC_kernel_size=None):
        ...
        if config.add_cross_attention:
            self.crossattention = TranceptionBlockAttention(
                ...
                SDWC_kernel_size=SDWC_kernel_size)
...
class TranceptionModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.position_embedding == "learned":
            ...
            # The softmax operation is invariant to translation, and bias functions are linear.
...
class TranceptionLMHeadModel(GPT2PreTrainedModel):
        ...
        if labels is not None:
            ...
            if self.retrieval_aggregation_mode is not None:
                ...
                if self.retrieval_aggregation_mode == "aggregate_indel":
                    ...
                    # shift_logits has one extra token (BOS) compared to truncated_sequence_text
                    if len(truncated_sequence_text) != shift_logits.shape[1] - 1:
                        print(
                            "Tokenization error -- seq length: {} and shift_logits length - 1 : {}"
                            .format(len(mutated_sequence),
                                    shift_logits.shape[1] - 1))
                    MSA_log_prior, MSA_start, MSA_end = msa_utils.update_retrieved_MSA_log_prior_indel(
                        self, self.MSA_log_prior, self.MSA_start, self.MSA_end, mutated_sequence[0])
                ...
                for seq_index in range(batch_size):
                    ...
                    if max_prior_slice <= min_prior_slice:
                        print(
                            f"Non overlapping region detected: min_prior_slice {min_prior_slice} "
                            f"and max_prior_slice {max_prior_slice}"
                        )
                        continue
                    ...
                    if flip[seq_index]:
                        ...
                        fused_shift_log_probas[seq_index, min_logits_slice:max_logits_slice, :] = (
                                1 - self.retrieval_inference_weight_RL
                            ) * shift_log_probas[seq_index, min_logits_slice:max_logits_slice, :] + \
                        ...
                        fused_shift_log_probas[
                            seq_index,
                            min_logits_slice:max_logits_slice, :] = (
                                1 - self.retrieval_inference_weight_LR
                            ) * shift_log_probas[
                                seq_index, min_logits_slice:
                                max_logits_slice, :] + self.retrieval_inference_weight_LR * slice_prior

                if self.retrieval_aggregation_mode == "aggregate_indel":
                    try:
                        # If a given residue colume is an added zero-column,
                        # then we overwrite prior fusion
                        # and only predict based on the autoregressive transformer inference mode.
                        inserted_retrieval_positions = [
                            True if slice_prior[i].sum() == 0 else False
                            for i in range(len(slice_prior))
                        ] + [True]  # Last True is for the end of sentence token
                        fused_shift_log_probas[:,
                                               inserted_retrieval_positions, :] = shift_log_probas[:,
                                                                                                   inserted_retrieval_positions, :]
                    except:
                        print(
                ...
                loss = loss_fct(input=fused_shift_log_probas.view(
                    -1, fused_shift_log_probas.size(-1)),
                                target=shift_labels.view(-1)).view(
                                    fused_shift_log_probas.shape[0],
                                    fused_shift_log_probas.shape[1])
    ...
    def _reorder_cache(past: Tuple[Tuple[torch.Tensor]],
        ...
        :meth:`~transformers.PreTrainedModel.beam_sample` is
        called. This is required to match :obj:`past_key_values` with the correct
    ...
    def score_mutants(self,
        ...
        Args:
            ...
            target_seq: (str) Full reference sequence (wild type) that is mutated in the DMS assay.
                        If not None, returned scores are delta log likelihood wrt that sequence.
            scoring_mirror: (bool) Whether to score mutated sequences from both directions
        ...
        if scoring_mirror:
            ...
            df_right_to_left_slices[
                'sliced_mutated_sequence'] = df_right_to_left_slices[
                    'sliced_mutated_sequence'].apply(lambda x: x[::-1])
            ...
            all_scores['avg_score'] = (all_scores['avg_score_L_to_R'] +
                                       all_scores['avg_score_R_to_L']) / 2.0
        else:
            all_scores = scores_L_to_R
            all_scores['avg_score'] = all_scores['avg_score_L_to_R']
        # By design "get_tranception_scores_mutated_sequences" drops the WT from the output.
        # We add it back if that was one of the sequences to score in the DMS (score=0 by default)
        if target_seq in DMS_data.mutated_sequence.values:
            if scoring_mirror:
                wt_row = pd.DataFrame([[target_seq, 0, 0, 0]],
                                      columns=[
                                          'mutated_sequence',
                                          'avg_score_L_to_R',
                                          'avg_score_R_to_L',
                ...
                wt_row = pd.DataFrame([[target_seq, 0, 0]],
                                      columns=[
                                          'mutated_sequence',
                                          'avg_score_L_to_R',
                                          'avg_score'
    ...
    def encode_batch(self,
                     protein_sequence,
                     sequence_name="sliced_mutated_sequence"):
        """
        Method to process an input AA sequence batch (protein_sequence)
        and return a tokenized sequence (via the tokenizer associated to the model).
        """
        ...
        return self.config.tokenizer(list(protein_sequence[sequence_name]),
