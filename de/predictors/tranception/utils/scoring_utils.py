...
def get_mutated_sequence(focus_seq, mutant, start_idx=1, AA_vocab=AA_vocab):
    ...
    for mutation in mutant.split(":"):
        ...
        assert (
            from_AA == focus_seq[relative_position]
        ), "Invalid from_AA or mutant position: " + str(
            mutation) + " from_AA: " + str(from_AA) + " relative pos: " + str(
                relative_position) + " focus_seq: " + str(focus_seq)
...
def get_tranception_scores_mutated_sequences(model,
                                             mutated_sequence_df,
    ...
    with torch.no_grad():
        ...
        for encoded_batch in ds_loader:
            ...
            window_start = np.array(
                mutated_sequence_df['window_start'][mutant_index:mutant_index +
                                                    full_batch_length])
            scores['window_start'] += list(window_start)
            window_end = np.array(
                mutated_sequence_df['window_end'][mutant_index:mutant_index +
                                                  full_batch_length])
            ...
            if (hasattr(model.config, "retrieval_aggregation_mode")) and (
                ...
                if reverse:
                    encoded_batch['flip'] = torch.tensor([1] *
                                                         full_batch_length)
                encoded_batch['start_slice'] = window_start
                encoded_batch['end_slice'] = window_end
                encoded_batch[
                    'mutated_sequence'] = mutated_sequence  # only mutated_sequence is flipped if the scoring_mirror branch of score_mutants. No need to flip mutated_sequence for MSA re-aligning
                ...
                loss = -loss_fct(
                    ...
                    target=shift_labels.view(-1)).view(shift_logits.shape[0],
    ...
    scores['score'] = scores['score'] / scores['mutated_sequence'].map(
        lambda x: len(x))
    if target_seq is not None:
        ...
        if model.config.scoring_window == "optimal":
            delta_scores = pd.merge(scores_mutated_seq,
                                    ...
                                    on=[merge_delta],
                                    suffixes=('', '_wt'))
            delta_scores[score_var_name] = delta_scores[
                'score'] - delta_scores['score_wt']
        elif model.config.scoring_window == "sliding":
            delta_scores = scores_mutated_seq.copy()
            delta_scores[score_var_name] = delta_scores['score'] - list(
                scores_wt['score']
            )[0]  # In sliding mode there is a single reference window for the WT
...
def get_sequence_slices(df,
    ...
    if scoring_window == "optimal":
        ...
        if indel_mode:  # For indels, we set the wild type reference to be always the same (full length) sequence. We assume here that the length is lower than model context size (otherwise "Sliding" mode should be used)
            df_wt['window_end'] = df_wt['mutated_sequence'].map(
                lambda x: len(x))
